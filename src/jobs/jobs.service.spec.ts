...
describe('Jobs (BullMQ & Scheduling)', () => {
  ...
  describe('JobsProcessor', () => {
    it('should invoke computeScores when processing compute-scores job', async () => {
      const computeScoresSpy = jest.spyOn(service, 'computeScores').mockResolvedValue({ processed: 0, updated: 0, errors: 0 } as any);
    ...
    it('should invoke computeReputation when processing compute-reputation job', async () => {
      const computeReputationSpy = jest.spyOn(service, 'computeReputation').mockResolvedValue({ processed: 0, updated: 0, errors: 0 } as any);
